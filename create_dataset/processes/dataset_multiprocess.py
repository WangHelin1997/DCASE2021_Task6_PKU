...
def create_dataset(settings_dataset: MutableMapping[str, Any],
    ...
    captions_evaluation = [
        csv_field.get(
            settings_dataset['annotations'][
                'captions_fields_prefix'].format(c_ind))
        for csv_field in csv_eva
        for c_ind in range(1, 6)]
    captions_validation = [
        csv_field.get(
            settings_dataset['annotations'][
                'captions_fields_prefix'].format(c_ind))
        for csv_field in csv_val
        for c_ind in range(1, 6)]
    all_captions.extend(captions_development)
    all_captions.extend(captions_evaluation)
    all_captions.extend(captions_validation)
    inner_logger.info('Done')
    ...
    words_list, chars_list = create_lists_and_frequencies(
        captions=all_captions, dir_root=dir_root,
        settings_ann=settings_dataset['annotations'],
    ...
    futures = []
    for split_data in [(csv_val, 'validation')]:
        futures.append(executor.submit(
            partial(_split, split_data, split_func, settings_dataset, settings_dirs_and_files,
                    inner_logger, dir_root, settings_audio_dirs)))
    [future.result() for future in tqdm(futures)]
...
def extract_features(root_dir: str,
    ...
    for data_file_name in filter(
            ...
            chain(dir_dev.iterdir(), dir_eva.iterdir(), dir_val.iterdir())):
        futures.append(executor.submit(
            partial(_extract, data_file_name, settings_features, settings_data, dir_output_dev, dir_output_eva, dir_output_val)))
    [future.result() for future in tqdm(futures)]


def _extract(data_file_name, settings_features, settings_data, dir_output_dev, dir_output_eva, dir_output_val):
    # Load the data file.
    ...
    if data_file_name.parent.name == settings_data['audio_dirs']['development']:
        parent_path = dir_output_dev
    elif data_file_name.parent.name == settings_data['audio_dirs']['evaluation']:
